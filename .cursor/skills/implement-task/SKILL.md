...
A path under `bug/` or `kind: bug` is the wrong job: stop and point to `/fix-bug`. No task file means plan it first or stop and ask. Commit each completed step; on close append Closed and a CHANGELOG Unreleased bullet. If spec was not edited, say `spec unchanged`. Do not merge or push to the default branch.
