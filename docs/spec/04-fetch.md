...
Все API-запросы идут на `{CONFLUENCE_BASE_URL}/rest/api/...`. База может включать контекст-путь. Префикс `/wiki` в `CONFLUENCE_BASE_URL` — признак Cloud и в первой версии не используется.
...
Пробы через ETag и прочие условные GET не требуются. Пропуск смотрит на локальные файлы и версию страницы в Confluence.
...
Ошибка одной страницы (403, 404, битый JSON, сбой скачивания тела) не отменяет запросы остальных страниц батча. Страницы обрабатываются последовательно в одном потоке.
