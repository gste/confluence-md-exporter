...
- Поток:
  ...
  5. В том же PR удалить закрытый файл и строку README. Когда слайсов не осталось — удалить `docs/todo/<epic-id>/`. Не оставлять тела задач как вторую спеку.
...
`feature` / `bugfix` — только префиксы git. Пустую подпапку не создавать. README эпика: таблицы **Task** и **Bug** (у Bug — колонка `Opened`). Нумерация своя в каждой подпапке. Имя файла — `NN-<slug>.md` без префикса `task`/`bug`.

Закрытый слайс удаляется **в том же PR**, что реализация: файл и строка README. Каталог эпика — когда в `task/` и `bug/` ничего не осталось.
...
7. В том же PR удалить закрытый файл из `task/` или `bug/` и строку README эпика; пустой эпик снести.
8. Если ревью меняет поведение → сначала правится спека, когда затронут контракт.
...
- [ ] закрытый файл очереди удалён в этом PR; `docs/todo/<epic>/` снесён, если слайсов не осталось
