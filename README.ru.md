...
### Ошибка: Команда не найдена / не является внутренней или внешней командой
Если после установки система не видит команду `confluence-md-exporter`, попробуйте следующие шаги:


1. **Альтернативный запуск:** Запустите утилиту напрямую через Python:
   ```bash
   python -m confluence_md_exporter -h
   ```
2. **Чистая переустановка (если первый шаг не помог):** Попробуйте запустить терминал от имени администратора и принудительно переустановить пакет:
   ```bash
